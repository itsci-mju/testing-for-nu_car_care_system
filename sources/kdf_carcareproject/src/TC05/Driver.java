...
public class Driver {
	...
	public String keyword_executor(String vKeyword, String vIP1, String vIP2) throws Exception {
		...
		public String getIP(String vIP){ 
			...
			if (vIP.equals("vCarSize")){
				vIP = StartUp.vCarSize;
			}
			if (vIP.equals("vCarType")){
				vIP = StartUp.vCarType;
			}
			if (vIP.equals("vCarNumber")){
				vIP = StartUp.vCarNumber;
			}
		...
		public void getData(int k) {
			...
			StartUp.vFirstName = StartUp.xTDdata[k][4];
			StartUp.vLastName = StartUp.xTDdata[k][5];
			StartUp.vEmail = StartUp.xTDdata[k][6];
			StartUp.vPhone = StartUp.xTDdata[k][7];
			StartUp.vUsername = StartUp.xTDdata[k][8];
			StartUp.vPassword = StartUp.xTDdata[k][9];
			StartUp.vBirthday = StartUp.xTDdata[k][10];
			StartUp.vHomeNumber = StartUp.xTDdata[k][11];
			StartUp.vStreet = StartUp.xTDdata[k][12];
			StartUp.vSubDistrict = StartUp.xTDdata[k][13];
			StartUp.vDistrict = StartUp.xTDdata[k][14];
			StartUp.vProvince = StartUp.xTDdata[k][15];
			StartUp.vZipCode = StartUp.xTDdata[k][16];
			StartUp.vCarSize = StartUp.xTDdata[k][17];
			StartUp.vCarType = StartUp.xTDdata[k][18];
			StartUp.vCarNumber = StartUp.xTDdata[k][19];
		}
	}
