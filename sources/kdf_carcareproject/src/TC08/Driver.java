...
public class Driver {
	...
	public String keyword_executor(String vKeyword, String vIP1, String vIP2) throws Exception {
		...
		public String getIP(String vIP){ 
			...
			if (vIP.equals("vUserLogin")){
				vIP = StartUp.vUserLogin;
			}
			if (vIP.equals("vPassLogin")){
				vIP = StartUp.vPassLogin;
			}
			if (vIP.equals("vSearch")){
				vIP = StartUp.vSearch;
			}
			if (vIP.equals("vOperation")){
				vIP = StartUp.vOperation;
			}
			if (vIP.equals("vPayment")){
				vIP = StartUp.vPayment;
			}
		...
		public void getData(int k) {
			...
			StartUp.vUserLogin = StartUp.xTDdata[k][4];
			StartUp.vPassLogin = StartUp.xTDdata[k][5];
			StartUp.vSearch = StartUp.xTDdata[k][6];
			StartUp.vOperation = StartUp.xTDdata[k][7];
			StartUp.vPayment = StartUp.xTDdata[k][8];
		}
	}
